...
save_path = os.path.join(save_dir, 'textcnn')
    ...
    def cnn(self):
        ...
        with tf.name_scope('cnn_layer'):
            ...
            filter_weights = tf.get_variable(name='weights',
                                             shape=[self.config.kernel_size, self.config.embedding_dim, 1,
            ...
            '''
            conv = tf.nn.conv2d(self.embeddings_inputs_expanded, filter_weights, strides=[1, 1, 1, 1], padding='VALID')
            ...
            pool = tf.nn.max_pool(conv, ksize=[1, self.config.seq_length - self.config.kernel_size + 1, 1, 1],
                                  strides=[1, 1, 1, 1], padding='VALID', name='pool')

            # 将池化的结果修改维度，修改成二维矩阵，因为第二维和第三维本身就是1，那么最后的结果为[batch_size,num_filters]
            h = tf.reshape(pool, [-1, self.config.num_filters])
            w = tf.get_variable(name='w', shape=[self.config.num_filters, self.config.num_classes],
